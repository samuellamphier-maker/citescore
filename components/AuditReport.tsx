...
import type { AuditReport as AuditReportData } from "@/lib/sample-report";
...
export function AuditReport({
  ...
  const label = kicker ?? (report.sample ? "CiteScore audit · Sample" : "CiteScore audit");
    ...
    <article className="report-sheet mx-auto max-w-[52rem] rounded-2xl border border-rule px-5 py-8 sm:px-10 sm:py-12 print:border-0 print:px-0 print:py-0">
      <header className="border-b border-rule pb-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className={`kicker ${report.sample ? "text-copper" : ""}`}>{label}</p>
          <p className="kicker">{report.auditedAt}</p>
        </div>
        <div className="mt-6 flex flex-col gap-8 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-lg text-ink-soft">{report.oneLiner}</p>
          ...
          <div className="text-center sm:text-right">
            ...
            <p className="mt-2 font-serif text-xl">{report.grade}</p>
          </div>
        </div>
      </header>
      ...
      <section className="border-b border-rule py-8">
        ...
        <p className="mt-3 text-[15px] leading-7 text-ink-soft">{report.summary}</p>
      </section>
      ...
      <section className="border-b border-rule py-8">
        ...
        {report.methodology ? (
          <p className="mt-4 text-sm leading-6 text-ink-soft">{report.methodology}</p>
        ) : null}
      </section>
        ...
        <ol className="mt-6 space-y-6">
          {report.fixes.map((fix) => (
            ...
            >
              ...
              <p className="mt-2 font-mono text-[11px] text-ink-soft">{fix.where}</p>
      ...
      <footer className="border-t border-rule pt-6 text-xs leading-5 text-ink-soft">
        {report.sample
          ? "This is a CiteScore sample report for a fictional product. Scores and recommendations are illustrative of format and depth, not a live crawl. A paid audit uses the same structure on your URL."
          : report.methodology}
      </footer>
    </article>
  );
